...
expName = 'adaptiveStroop'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\github\\study2\\stroop-task\\stroop(adaptive)_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    _moveOn_2_allKeys = []
    # keep track of which components have finished
    Instruction2Components = [instImage2, instText_2, moveOn_2]
    ...
    while continueRoutine:
        ...
        # if instImage2 is starting this frame...
        if instImage2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instImage2.frameNStart = frameN  # exact frame index
            instImage2.tStart = t  # local t and not account for scr refresh
            instImage2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instImage2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'instImage2.started')
            # update status
            instImage2.status = STARTED
            instImage2.setAutoDraw(True)
        
        # if instImage2 is active this frame...
        if instImage2.status == STARTED:
            # update params
            pass
    ...
    for thisExperimentalBlock in experimentalBlock:
        ...
        # adaptation
        if sequenceDataset.congruent[experimentalBlock.thisN]==0:   # for "incongruent" trials
            if sequenceDataset.Accuracy[experimentalBlock.thisN]==1:
                sequenceDataset.step[experimentalBlock.thisN]    = -0.05    # if correct response, decrease duration
            else:
                sequenceDataset.step[experimentalBlock.thisN]    = +0.05    # otherwise (incorrect or no response), increase duration
        else:  # for "congruent" trials
        ...
        # Run 'Begin Routine' code from fbcCode
