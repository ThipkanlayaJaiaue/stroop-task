...
expName = 'controlStroop'  # from the Builder filename that created this script
expInfo = {
    'participant': '',
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\github\\study2\\stroop-task\\stroop(control)_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    startText = visual.TextStim(win=win, name='startText',
        ...
        font='Open Sans',
        pos=None, height=0.1, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    ...
    fbCound = sound.Sound('600', secs=0.050, stereo=True, hamming=False,
        name='fbCound')
    fbCound.setVolume(1.0)
    ...
    for thisExperimentalBlock in experimentalBlock:
        ...
        fbCound.setSound('600', secs=0.050, hamming=False)
        fbCound.setVolume(sound_volume, log=False)
        fbCound.seek(0)
        # keep track of which components have finished
        blankComponents = [blankPeriod, progressBar_2, fbCound]
        ...
        while continueRoutine:
            ...
            # if fbCound is starting this frame...
            if fbCound.status == NOT_STARTED and t >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                fbCound.frameNStart = frameN  # exact frame index
                fbCound.tStart = t  # local t and not account for scr refresh
                fbCound.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('fbCound.started', t)
                # update status
                fbCound.status = STARTED
                fbCound.play()  # start the sound (it finishes automatically)
            
            # if fbCound is stopping this frame...
            if fbCound.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fbCound.tStartRefresh + 0.050-frameTolerance:
                    # keep track of stop time/frame for later
                    fbCound.tStop = t  # not accounting for scr refresh
                    fbCound.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.addData('fbCound.stopped', t)
                    # update status
                    fbCound.status = FINISHED
                    fbCound.stop()
            # update fbCound status according to whether it's playing
            if fbCound.isPlaying:
                fbCound.status = STARTED
            elif fbCound.isFinished:
                fbCound.status = FINISHED
            # *blankPeriod* period
